  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-(--color-text-primary)">Runs</h1>
          <p className="mt-1 text-sm text-(--color-text-secondary)">Execution attempts for your experiments.</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-(--color-text-secondary)">Experiment:</label>
        <select value={experimentFilter} onChange={(e) => setExperimentFilter(e.target.value)} className="rounded-md border border-(--color-border) px-3 py-2 text-sm">
      ...
      {loading ? (
        <div className="p-8 text-center text-sm text-(--color-muted)">Loading runs…</div>
      ) : (
        <DataTable
          columns={[
      ...
      {open && (
        <FormModal title="New Run" onClose={() => setOpen(false)} onSubmit={onSubmit}>
          <FormField label="Experiment" required>
            <select className="mt-1 w-full rounded-md border border-(--color-border) px-3 py-2 text-sm" value={form.experiment_id} onChange={(e) => setForm({ ...form, experiment_id: e.target.value })}>
          ...
          <FormField label="Model Name" required>
            <input className="mt-1 w-full rounded-md border border-(--color-border) px-3 py-2 text-sm" value={form.model_name} onChange={(e) => setForm({ ...form, model_name: e.target.value })} />
          </FormField>
