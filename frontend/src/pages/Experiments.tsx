    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Group runs under a dataset and research question.</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-gray-700">Filter by dataset:</label>
        <select value={datasetFilter} onChange={(e) => setDatasetFilter(e.target.value)} className="rounded-md border border-gray-300 px-3 py-2 text-sm">
      ...
      {loading ? (
        <div className="p-8 text-center text-sm text-gray-500">Loading experiments…</div>
      ) : (
        <DataTable
          columns={[
      ...
      {open && (
        <FormModal title="New Experiment" onClose={() => setOpen(false)} onSubmit={onSubmit}>
          <FormField label="Dataset" required>
            <select className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.dataset_id} onChange={(e) => setForm({ ...form, dataset_id: e.target.value })}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Objective">
              <textarea className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.objective} onChange={(e) => setForm({ ...form, objective: e.target.value })} />
            </FormField>
            <FormField label="Task">
              <input className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.task} onChange={(e) => setForm({ ...form, task: e.target.value })} />
            </FormField>
          </div>
