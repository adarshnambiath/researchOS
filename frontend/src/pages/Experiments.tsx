  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-[var(--color-text-secondary)]">Filter by dataset:</label>
        <select value={datasetFilter} onChange={(e) => setDatasetFilter(e.target.value)} className="rounded-md border border-[var(--color-border)] px-3 py-2 text-sm">
      ...
      {loading ? (
        <div className="p-8 text-center text-sm text-[var(--color-muted)]">Loading experiments…</div>
      ) : (
        <DataTable
          columns={[
      ...
      {open && (
        <FormModal title="New Experiment" onClose={() => setOpen(false)} onSubmit={onSubmit}>
          <FormField label="Dataset" required>
            <select className="mt-1 w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm" value={form.dataset_id} onChange={(e) => setForm({ ...form, dataset_id: e.target.value })}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Objective">
              <textarea className="mt-1 w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm" value={form.objective} onChange={(e) => setForm({ ...form, objective: e.target.value })} />
            </FormField>
            <FormField label="Task">
              <input className="mt-1 w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm" value={form.task} onChange={(e) => setForm({ ...form, task: e.target.value })} />
            </FormField>
          </div>
